...
if (($_POST['action'] ?? '') === 'create_campaign') {
    try {
        $mp->createCampaign($_POST);
        $success_message = 'Kampanya eklendi';
    } catch (Exception $e) { $error_message = $e->getMessage(); }
}

$list = $mp->listCampaigns(1, 100);
?>
<!DOCTYPE html>
<html lang="tr">
<head>
...
<body class="bg-light">
    <div class="container-fluid p-3">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="mb-0"><i class="fas fa-bullhorn text-primary"></i> Kampanyalar</h4>
        </div>
        ...
        <div class="row">
            <div class="col-md-5">
                <div class="card">
                    <div class="card-header">Yeni Kampanya</div>
                    <div class="card-body">
                        <form method="post">
                            <input type="hidden" name="action" value="create_campaign">
                            <div class="mb-2"><label class="form-label">Adı</label><input type="text" name="kampanya_adi" class="form-control" required></div>
                            <div class="mb-2"><label class="form-label">Açıklama</label><input type="text" name="aciklama" class="form-control"></div>
                            <div class="row">
            ...
            <div class="col-md-7">
                <div class="card">
                    <div class="card-header">Kayıtlı Kampanyalar</div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-sm">
                                <thead><tr><th>ID</th><th>Ad</th><th>Baş</th><th>Bit</th><th>Aktif</th></tr></thead>
                                <tbody>
                                <?php foreach (($list['items'] ?? []) as $c): ?>
                                    <tr>
                                        <td><?php echo (int)$c['id']; ?></td>
                                        <td><?php echo htmlspecialchars($c['kampanya_adi']); ?></td>
                                        <td><?php echo htmlspecialchars((string)$c['baslangic_tarihi']); ?></td>
                                        <td><?php echo htmlspecialchars((string)$c['bitis_tarihi']); ?></td>

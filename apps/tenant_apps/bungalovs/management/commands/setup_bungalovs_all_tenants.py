"""
Tüm tenant schema'larında bungalovs modülünü kur
...
class Command(BaseCommand):
    help = 'Tüm tenant schema\'larında bungalovs modülünü kur (migration + permission)'
    ...
    def handle(self, *args, **options):
        ...
        if not skip_public:
            ...
            try:
                ...
                call_command('create_bungalovs_module', verbosity=1)
        ...
        if not skip_migration and not skip_public:
            ...
            try:
                connection.set_schema_to_public()
                call_command('migrate', 'bungalovs', verbosity=1)
        ...
        for tenant in tenants:
            try:
                ...
                if not skip_migration:
                    ...
                    try:
                        with schema_context(tenant.schema_name):
                            call_command('migrate', 'bungalovs', verbosity=0)
                ...
                if not skip_permission:
                    ...
                    try:
                        with schema_context(tenant.schema_name):
                            call_command('create_bungalovs_permissions', verbosity=0)
                        self.stdout.write(self.style.SUCCESS('OK'))
                    except Exception as e:
                        self.stdout.write(self.style.ERROR(f'HATA: {str(e)}'))
                        raise

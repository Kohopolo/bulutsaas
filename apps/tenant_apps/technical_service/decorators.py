"""
Teknik Servis Permission Decorators
"""
...
def require_technical_service_permission(permission_level='view'):
    """Teknik Servis modülü yetki kontrolü decorator'ı"""
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            try:
                ...
                is_admin = tenant_user.has_module_permission('technical_service', 'admin')
                if is_admin:
                    return view_func(request, *args, **kwargs)
                
                if not tenant_user.has_module_permission('technical_service', permission_code):
                    if permission_code == 'view':
                        ...
                        if active_subscription:
                            try:
                                module = Module.objects.get(code='technical_service')
                                ...
                                if not package_module:
                                    messages.error(request, 'Teknik Servis modülü paketinizde aktif değil.')
                ...
                if not hotel_permission:
                    if tenant_user.has_module_permission('technical_service', 'view'):
                        if permission_level == 'view':
                            return view_func(request, *args, **kwargs)
                        else:
                            messages.error(request, f'{hotel.name} oteli için teknik servis yetkiniz bulunmamaktadır.')
                            return redirect('hotels:select_hotel')
                    else:
                        messages.error(request, f'{hotel.name} oteli için teknik servis yetkiniz bulunmamaktadır.')
                        return redirect('hotels:select_hotel')
                ...
                if user_level >= required_level:
                    ...
                    return redirect('technical_service:dashboard')
                ...
                logger.error(f'Technical Service yetki kontrolü hatası: {str(e)}', exc_info=True)

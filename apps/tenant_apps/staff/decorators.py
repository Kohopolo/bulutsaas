...
def require_staff_permission(permission_level='view'):
    """Personel Yönetimi modülü yetki kontrolü decorator'ı"""
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            try:
                ...
                is_admin = tenant_user.has_module_permission('staff', 'admin')
                if is_admin:
                    return view_func(request, *args, **kwargs)
                
                if not tenant_user.has_module_permission('staff', permission_code):
                    if permission_code == 'view':
                        ...
                        if active_subscription:
                            try:
                                module = Module.objects.get(code='staff')
                                ...
                                if not package_module:
                                    messages.error(request, 'Personel Yönetimi modülü paketinizde aktif değil.')
                ...
                if not hotel_permission:
                    if tenant_user.has_module_permission('staff', 'view'):
                        if permission_level == 'view':
                            return view_func(request, *args, **kwargs)
                        else:
                            messages.error(request, f'{hotel.name} oteli için personel yönetimi yetkiniz bulunmamaktadır.')
                            return redirect('hotels:select_hotel')
                    else:
                        messages.error(request, f'{hotel.name} oteli için personel yönetimi yetkiniz bulunmamaktadır.')
                        return redirect('hotels:select_hotel')
                ...
                logger.error(f'Staff yetki kontrolü hatası: {str(e)}', exc_info=True)

"""
Muhasebe Modülü Decorator'ları
"""
...
def require_accounting_module(view_func):
    """
    Muhasebe modülünün aktif olup olmadığını kontrol eder
    """
    @wraps(view_func)
    def _wrapped_view(request, *args, **kwargs):
        ...
        try:
            ...
            if not package_module:
                messages.error(request, 'Muhasebe modülü paketinizde aktif değil.')
                return redirect('tenant:dashboard')
        except Module.DoesNotExist:
            messages.error(request, 'Muhasebe modülü bulunamadı.')
            return redirect('tenant:dashboard')

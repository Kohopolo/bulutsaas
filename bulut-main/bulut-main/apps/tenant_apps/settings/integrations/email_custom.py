"""
Custom SMTP Email Gateway Integration
"""
...
class CustomSMTPEmailGateway(BaseEmailGateway):
    """
    Custom SMTP Email Gateway
    Herhangi bir SMTP sunucusu için genel gateway
    """
    ...
    def send_email(
        ...
        """
        Custom SMTP üzerinden email gönder
        """
        try:
            # Host kontrolü
            if not self.host:
                return {
                    'success': False,
                    'error': 'SMTP host belirtilmedi',
                    'message': 'SMTP host adresi gereklidir'
                }

"""
Kalite Kontrol Permission Decorators
"""
...
def require_quality_control_permission(permission_level='view'):
    """Kalite Kontrol modülü yetki kontrolü decorator'ı"""
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            try:
                ...
                is_admin = tenant_user.has_module_permission('quality_control', 'admin')
                if is_admin:
                    return view_func(request, *args, **kwargs)
                
                if not tenant_user.has_module_permission('quality_control', permission_code):
                    if permission_code == 'view':
                        ...
                        if active_subscription:
                            try:
                                module = Module.objects.get(code='quality_control')
                                ...
                                if not package_module:
                                    messages.error(request, 'Kalite Kontrol modülü paketinizde aktif değil.')
                ...
                if not hotel_permission:
                    if tenant_user.has_module_permission('quality_control', 'view'):
                        if permission_level == 'view':
                            return view_func(request, *args, **kwargs)
                        else:
                            messages.error(request, f'{hotel.name} oteli için kalite kontrol yetkiniz bulunmamaktadır.')
                            return redirect('hotels:select_hotel')
                    else:
                        messages.error(request, f'{hotel.name} oteli için kalite kontrol yetkiniz bulunmamaktadır.')
                        return redirect('hotels:select_hotel')
                ...
                logger.error(f'Quality Control yetki kontrolü hatası: {str(e)}', exc_info=True)

"""
NestPay Payment Gateway Integration
(İş Bankası, Garanti, Akbank, Ziraat, Yapı Kredi vb.)
"""
import hashlib
import base64
import requests
...
class NestPayGateway(BasePaymentGateway):
    """NestPay Payment Gateway (NestPay API)"""
    
    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        # Banka bazlı URL'ler
        self.bank_urls = {
            'isbank': 'https://sanalpos.isbank.com.tr',
            'garanti': 'https://sanalposprov.garanti.com.tr',
            'akbank': 'https://www.sanalakpos.com',
            'ziraat': 'https://sanalpos.ziraatbank.com.tr',
            'yapikredi': 'https://posnet.yapikredi.com.tr',
        }
        self.bank_code = config.get('bank_code', 'isbank')
        self.base_url = self.bank_urls.get(self.bank_code, self.bank_urls['isbank'])
    
    def _generate_hash(self, data: Dict[str, Any]) -> str:
        """Generate hash for NestPay"""
    ...
    def verify_payment(self, transaction_id: str, **kwargs) -> Dict[str, Any]:
        ...
        # NestPay callback'ten gelen bilgileri doğrula

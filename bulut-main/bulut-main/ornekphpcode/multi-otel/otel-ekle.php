<?php
/**
 * Multi Otel - Yeni Otel Ekleme
 * Yeni otel ekleme formu
 */
...
requireDetailedPermission('otel_ekle', 'Otel ekleme yetkiniz bulunmamaktadır.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_otel'])) {
    ...
    if (!validateCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            if (empty($errors)) {
                // Otel ekle
                $sql = "INSERT INTO oteller (otel_adi, kisa_aciklama, adres, telefon, email, website, sira_no, durum) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                
                if (executeQuery($sql, [$otel_adi, $kisa_aciklama, $adres, $telefon, $email, $website, $sira_no, $durum])) {
                    $otel_id = $pdo->lastInsertId();
                    
                    // Varsayılan otel ayarlarını ekle
                    $default_settings = [
                        ['checkin_saati', '14:00', 'Check-in saati', 'genel'],
                        ['checkout_saati', '12:00', 'Check-out saati', 'genel'],
                        ['min_rezervasyon_gunu', '1', 'Minimum rezervasyon günü', 'genel'],
                        ['max_rezervasyon_gunu', '30', 'Maksimum rezervasyon günü', 'genel'],
                        ['erken_checkin_ucreti', '0', 'Erken check-in ücreti', 'fiyat'],
                        ['gec_checkout_ucreti', '0', 'Geç check-out ücreti', 'fiyat'],
                        ['iptal_politikasi', '24', 'İptal politikası (saat)', 'genel']
                    ];
                    
                    foreach ($default_settings as $setting) {
                        executeQuery("INSERT INTO otel_ayarlari (otel_id, anahtar, deger, aciklama, kategori) VALUES (?, ?, ?, ?, ?)", 
                            [$otel_id, $setting[0], $setting[1], $setting[2], $setting[3]]);
                    }
                    
                    $success_message = 'Otel başarıyla eklendi.';
                    
                    // Otel listesine yönlendir
                    header('Location: oteller.php?success=' . urlencode($success_message));
                    exit;
                } else {
                    $error_message = 'Otel eklenirken hata oluştu.';
                }
            } else {
...
<head>
    ...
    <title>Yeni Otel Ekle - Multi Otel Yönetimi</title>
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h1 class="h3 mb-0">Yeni Otel Ekle</h1>
                            <p class="text-muted">Sisteme yeni otel ekleyin</p>
                        </div>
                ...
                <div class="card-body">
                    <form method="POST" action="">
                        ...
                        <div class="row">
                            <div class="col-md-8 mb-3">
                                ...
                                <input type="text" class="form-control" id="otel_adi" name="otel_adi" 
                                       value="<?php echo htmlspecialchars($_POST['otel_adi'] ?? ''); ?>" required>
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="kisa_aciklama" name="kisa_aciklama" rows="3"><?php echo htmlspecialchars($_POST['kisa_aciklama'] ?? ''); ?></textarea>
                        </div>
                        
                        <div class="mb-3">
                            <label for="adres" class="form-label">Adres</label>
                            <textarea class="form-control" id="adres" name="adres" rows="2"><?php echo htmlspecialchars($_POST['adres'] ?? ''); ?></textarea>
                        </div>
                        ...
                        <div class="d-flex justify-content-end">
                            ...
                            <button type="submit" name="submit_otel" class="btn btn-primary">
                                <i class="fas fa-save me-2"></i>Otel Ekle
                            </button>
                        </div>
                    </form>
                </div>
